...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const checkInstalled = () => {
      // Check for standalone mode (PWA installed) - works for both iOS and Android
      ...
      // For iOS, check multiple indicators of PWA installation
      if (isIOSDevice) {
        ...
        // Hide install button if app is running as PWA
        if (isStandalone || isStandaloneMode || isFullscreen) {
          setIsInstallable(false);
          setIsPWA(true);
        } else {
          setIsInstallable(true);
      ...
      // For Android Chrome, show install button if not in standalone mode
      if (isAndroid && isChrome) {
        const isStandaloneMode = window.matchMedia && window.matchMedia('(display-mode: standalone)').matches;
        const isFullscreen = window.matchMedia && window.matchMedia('(display-mode: fullscreen)').matches;
        
        if (isStandaloneMode || isFullscreen) {
          setIsInstallable(false);
          setIsPWA(true);
        } else {
  ...
  const handleInstallClick = async () => {
    // For iOS, show the installation guide
    if (isIOS) {
    ...
    if (deferredPrompt) {
      ...
      // Fallback for Android Chrome when deferredPrompt is not available
      const isAndroid = /Android/.test(navigator.userAgent);
      const isChrome = /Chrome/.test(navigator.userAgent);
      
      if (isAndroid && isChrome) {
        // Show a message to use Chrome's menu
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="border-b border-border/30 bg-card/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
          ...
          <img 
            src="/hestia-icon.svg" 
            alt="Hestia Logo" 
            className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl object-cover glow-primary"
          />
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground">
                  Saint Joseph College • Chatbot Assistant
                  {isPWA && (
            ...
            <div className="flex gap-1 sm:gap-2">
              ...
              {isInstallable && (
                <Button
                  ...
                  size="sm"
                  className="glow-primary bg-green-600 hover:bg-green-700 text-white transition-all duration-300"
                >
                  <Download size={16} className="mr-1 sm:mr-2" />
                  <span className="hidden sm:inline">{isIOS ? 'Add to Home' : 'Install PWA'}</span>
                  <span className="sm:hidden">{isIOS ? 'Add' : 'App'}</span>
                </Button>
              )}
            </div>
          </div>
      ...
      {/* Hero Section - Show only when FAQ tab is active */}
      {activeTab === 'faq' && (
        <section className="py-8 sm:py-12 px-4 sm:px-6 bg-gradient-to-b from-background to-background/50">
            ...
            <div className="flex flex-col gap-4 justify-center items-center mb-6 sm:mb-8">
              ...
              {!isIOS && (
                <Button
                  ...
                  size="lg"
                  onClick={handleInstallClick}
                  className="glow-primary bg-green-600 hover:bg-green-700 text-white transition-all duration-300 w-full sm:w-auto min-w-48"
                >
                  <Download size={20} className="mr-2" />
                  {deferredPrompt ? 'Install PWA' : 'Install App'}
                </Button>
              )}
      ...
      <main className="flex-1 flex flex-col">
        <section className={`${activeTab === 'chat' ? 'pt-4 sm:pt-6' : 'py-6 sm:py-8'} px-4 sm:px-6 flex-1`}>
      ...
      {isInstallable && (
        <div className="fixed bottom-20 right-4 z-50 sm:hidden">
          <Button
            ...
            size="sm"
            onClick={handleInstallClick}
            className="glow-primary bg-green-600 hover:bg-green-700 text-white shadow-lg"
          >
            <Download size={16} className="mr-2" />
            {isIOS ? 'Add to Home' : 'Install PWA'}
          </Button>
        </div>
      )}
      ...
      {/* Footer */}
      <footer className="border-t border-border/30 bg-card/30 backdrop-blur-sm py-6 sm:py-8 px-4 sm:px-6 mt-auto">
        <div className="container mx-auto text-center">
          <p className="text-sm sm:text-base text-muted-foreground mb-2">
            © 2025 Tourism & Hospitality Department • Saint Joseph College
          </p>
